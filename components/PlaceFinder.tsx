...
export const PlaceFinder: React.FC = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setLocationError(
            "Location access denied. 'Nearest' search may be less accurate."
          );
        }
      );
    }
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    const searchQuery = query.toLowerCase().includes("seoul")
      ? query
      : `${query} in Seoul, Korea`;
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold text-gray-900">
          Find Best Eats in Seoul
        </h2>
        <p className="text-gray-600">
          Powered by Google Maps & Search Grounding
        </p>
      </div>
      ...
      <form onSubmit={handleSearch} className="relative">
        ...
        {locationError && (
          <p className="text-xs text-amber-600 mt-2 ml-1">{locationError}</p>
        )}
      </form>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-12 space-y-4 animate-fade-in">
          ...
          <p className="text-gray-500 font-medium">
            Scouring the map for delicious spots...
          </p>
        </div>
      )}
